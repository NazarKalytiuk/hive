...
**Phase L1 COMPLETE**, **Phase L2 COMPLETE**, and **Phase L3 COMPLETE**. L1/L2 are fully shipped (diagnostics, hover, completion, symbols, definition, references, rename, code lens). Phase L3 has landed all six tickets — L3.1 formatting (NAZ-302), L3.2 code actions + extract env var (NAZ-303), L3.3 capture-field + scaffold-assert code actions (NAZ-304), L3.4 fix-plan Quick Fix via shared `tarn::fix_plan` library (NAZ-305), L3.5 nested schema completion (NAZ-306), and L3.6 JSONPath evaluator hover + `workspace/executeCommand` (NAZ-307). Epic NAZ-301 is closed. **Phase V** (VS Code extension migration onto `tarn-lsp`) is the next coordinated initiative under a new epic.
...
| L3.3 | Two new code-action providers plug into the L3.2 dispatcher: **capture this field** lifts a JSONPath literal inside `assert.body:` into a new `capture:` entry (leaf-name derivation, collision suffix, merge into existing capture block), and **scaffold assert.body from last response** walks the top-level fields of a recorded response (pluggable `RecordedResponseSource` trait, disk reader + in-memory mock) and emits an `assert.body` block pre-populated with inferred types. Introduces the NAZ-304 sidecar convention `<file>.last-run/<test-slug>/<step-slug>.response.json` — reader ships now, writer is a separate follow-up. **[shipped — NAZ-304]** | 7 | M | L3.2 | `tarn-lsp/src/code_actions/mod.rs`, `tarn-lsp/src/code_actions/capture_field.rs`, `tarn-lsp/src/code_actions/scaffold_assert.rs`, `tarn-lsp/src/code_actions/jsonpath_name.rs`, `tarn-lsp/src/code_actions/response_source.rs`, `tarn-lsp/src/code_actions/extract_env.rs`, `tarn-lsp/tests/code_actions_test.rs`, `tarn-lsp/Cargo.toml`, `docs/TARN_LSP.md` |
| L3.4 | New shared library surface `tarn::fix_plan` with two constructors: `generate_fix_plan_from_report` (report-driven, advice only — the path the MCP `tarn_fix_plan` tool has always used, now lifted out of `tarn-mcp`) and `generate_fix_plan` (diagnostic-driven, structured edits — the new path backing the LSP Quick Fix). The LSP side adds the fourth code-action provider, **Apply fix** (`CodeActionKind::QUICKFIX`), which recognises the parser's stable `Unknown field 'X' at <context>. Did you mean 'Y'?` messages and emits a single-key replacement edit at the typo'd YAML key's exact source position via a `yaml-rust2` marked-event walker. `tarn-mcp`'s `tarn_fix_plan` tool now delegates to the library and its golden contract passes byte-for-byte. **[shipped — NAZ-305]** | 8 | M | L3.3 | `tarn/src/fix_plan.rs`, `tarn/src/lib.rs`, `tarn-mcp/src/tools.rs`, `tarn-lsp/src/code_actions/mod.rs`, `tarn-lsp/src/code_actions/quick_fix.rs`, `tarn-lsp/tests/code_actions_test.rs`, `docs/TARN_LSP.md` |
| L3.5 | Nested schema completion: extend the NAZ-293 completion provider with a JSON-Schema tree walker that descends through `properties`, `items`, `additionalProperties`, local `$ref`, and `oneOf`/`anyOf`/`allOf`, plus a permissive line-based YAML walker that maps the cursor to a `SchemaPath`. Offers schema-appropriate child keys inside `request.*`, `assert.*`, `assert.body."$.id".*` (BodyAssertionOperators: `eq`/`gt`/`matches`/`length`/`type`/`is_uuid`/…), `capture.<name>.*` (ExtendedCapture keys), and `poll.*` (PollConfig fields), with the schema's `description` surfaced as completion `documentation`. **[shipped — NAZ-306]** | 7 | M | L3.4 | `tarn-lsp/src/schema.rs`, `tarn-lsp/src/completion.rs`, `tarn-lsp/tests/completion_test.rs`, `tarn-lsp/Cargo.toml`, `docs/TARN_LSP.md` |
| L3.6 | JSONPath evaluator — hover + `workspace/executeCommand`. New shared library surface `tarn::jsonpath::evaluate_path` as a thin canonical wrapper over `serde_json_path`. Hover provider grows a fifth token class (JSONPath literal) that evaluates `$.foo` against the step's last recorded response via the L3.3 `RecordedResponseSource` trait and appends the result as pretty-printed JSON to the hover markdown (capped at 2000 chars). Server advertises `execute_command_provider` with one stable command `tarn.evaluateJsonpath`; accepts `{ path, response }` inline or `{ path, step: { file, test, step } }` and returns `{ matches: [...] }`. Ships the last Phase L3 capability — Epic NAZ-301 now closed. **[shipped — NAZ-307]** | 7 | M | L3.5 | `tarn/src/jsonpath.rs`, `tarn/src/lib.rs`, `tarn-lsp/src/jsonpath_eval.rs`, `tarn-lsp/src/hover.rs`, `tarn-lsp/src/server.rs`, `tarn-lsp/src/capabilities.rs`, `tarn-lsp/src/lib.rs`, `tarn-lsp/tests/jsonpath_test.rs`, `tarn-lsp/tests/initialize_test.rs`, `docs/TARN_LSP.md` |
