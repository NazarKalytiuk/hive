...
**Phase L1 COMPLETE**, **Phase L2 COMPLETE**, and **Phase L3 in progress**. L1/L2 are fully shipped (diagnostics, hover, completion, symbols, definition, references, rename, code lens). Phase L3 has landed its first three tickets — L3.1 formatting (NAZ-302), L3.2 code actions + extract env var (NAZ-303), and L3.3 capture-field + scaffold-assert code actions (NAZ-304). Remaining L3 tickets (fix-plan quick fix, nested completion, inline JSONPath evaluator) are tracked under Epic NAZ-301.
