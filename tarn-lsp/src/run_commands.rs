...
pub fn execute_run_last_failures(
    ...
    let env_override = args
        .env
        .clone()
        .or_else(|| last_run.env_name.clone());
    ...
    for failure in &failures {
        ...
        let run =
            run_file_internal(&file_path, Some(vec![selector]), env_override.as_deref(), sink)?;
...
fn run_file_internal(
    ...
    let mut test_file: TestFile = parser::parse_file(file_path).map_err(|e| {
...
fn finalize(
    ...
    let json = tarn::report::json::render_with_mode(&run, tarn::report::json::JsonOutputMode::Verbose);
    let data: Value =
