...
/// CLI --var > shell env ${VAR} > hive.env.local.yaml > hive.env.{name}.yaml > hive.env.yaml > inline env: block
pub fn resolve_env(
    ...
    // Layer 2: hive.env.yaml (default env file)
    let default_env_file = base_dir.join("hive.env.yaml");
    ...
    // Layer 3: hive.env.{name}.yaml (environment-specific)
    if let Some(name) = env_name {
        let named_env_file = base_dir.join(format!("hive.env.{}.yaml", name));
    ...
    // Layer 4: hive.env.local.yaml (gitignored, secrets)
    let local_env_file = base_dir.join("hive.env.local.yaml");
...
fn load_env_file(path: &Path) -> Result<HashMap<String, String>, HiveError> {
    let content = std::fs::read_to_string(path).map_err(|e| {
        HiveError::Config(format!("Failed to read env file {}: {}", path.display(), e))
    })?;
...
pub fn parse_cli_vars(vars: &[String]) -> Result<Vec<(String, String)>, HiveError> {
    vars.iter()
        .map(|s| {
...
mod tests {
    ...
    fn env_file_overrides_inline() {
        let dir = TempDir::new().unwrap();
        setup_env_files(&dir, &[("hive.env.yaml", "base_url: http://from-file")]);
    ...
    fn named_env_overrides_default() {
        ...
        setup_env_files(
            &dir,
            &[
                ("hive.env.yaml", "base_url: http://default"),
                ("hive.env.staging.yaml", "base_url: http://staging"),
            ],
        );
    ...
    fn local_env_overrides_named() {
        ...
        setup_env_files(
            ...
            &[
                ("hive.env.yaml", "base_url: http://default"),
                ("hive.env.staging.yaml", "base_url: http://staging"),
                ("hive.env.local.yaml", "base_url: http://local"),
            ],
        );
    ...
    fn variables_from_different_layers_merge() {
        ...
        setup_env_files(
            &dir,
            &[("hive.env.yaml", "file_only: from_file\nbase_url: from_file")],
        );
    ...
    fn missing_named_env_file_is_ok() {
        let dir = TempDir::new().unwrap();
        // No hive.env.staging.yaml exists
        let env = resolve_env(&HashMap::new(), Some("staging"), &[], dir.path()).unwrap();
        assert!(env.is_empty());
    }
}
