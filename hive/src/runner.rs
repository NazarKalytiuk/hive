...
pub fn run_file(
    ...
    // Run setup steps
    let setup_results = run_steps(&test_file.setup, env, &mut captures, test_file)?;
    ...
    if !setup_failed {
        if !test_file.steps.is_empty() {
            ...
            let step_results = run_steps(&test_file.steps, env, &mut step_captures, test_file)?;
        ...
        for (name, test_group) in &test_file.tests {
            ...
            let step_results = run_steps(&test_group.steps, env, &mut test_captures, test_file)?;
            let passed = step_results.iter().all(|s| s.passed);
            let duration_ms = step_results.iter().map(|s| s.duration_ms).sum();
            test_results.push(TestResult {
                name: name.clone(),
    ...
    let teardown_results = run_steps(&test_file.teardown, env, &mut captures, test_file)?;
...
fn run_steps(
    ...
    for step in steps {
        let result = run_step(step, env, captures, test_file)?;
        results.push(result);
    }
...
fn run_step(
    ...
    // Get timeout from defaults
    let timeout = test_file.defaults.as_ref().and_then(|d| d.timeout);
