...
/// Project-level configuration from hive.config.yaml.
#[derive(Debug, Deserialize, Clone)]
pub struct HiveConfig {
    /// Directory to scan for .hive.yaml test files
...
/// Load project config from hive.config.yaml.
/// Returns default config if the file doesn't exist.
pub fn load_config(base_dir: &Path) -> Result<HiveConfig, HiveError> {
    let config_path = base_dir.join("hive.config.yaml");
    if !config_path.exists() {
        return Ok(HiveConfig::default());
    }

    let content = std::fs::read_to_string(&config_path).map_err(|e| {
        HiveError::Config(format!("Failed to read {}: {}", config_path.display(), e))
    })?;

    let config: HiveConfig = serde_yaml::from_str(&content).map_err(|e| {
        HiveError::Config(format!("Failed to parse {}: {}", config_path.display(), e))
    })?;
...
mod tests {
    ...
    fn default_config() {
        ...
        assert_eq!(config.env_file, "hive.env.yaml");
    ...
    fn load_config_with_all_fields() {
        let dir = TempDir::new().unwrap();
        let config_path = dir.path().join("hive.config.yaml");
